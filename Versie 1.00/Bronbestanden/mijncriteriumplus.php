        ...
        <form action="mijncriteriumins.php" method="POST">
            <div class="form-group form-inline">
                <select class="form-control" id="CrVanTem" name="CrVanTem">
                    ...
                    <?php foreach ($crtsvantem as $crvantem) { ?>
                        <option value="<?= $crvantem['ID'] ?>"><?= $crvantem['Criterium'] ?></option>
                    <?php } ?>
                </select>
                <label for="Van" class="spacer-left">Van:</label>
                <input type="number" size=7 class="form-control" min=0 id="Van" name="Van">
                <label for="Tem" class="spacer-left">T/m:</label>
                <input type="number" size=7 class="form-control" min=0 id="Tem" name="Tem">
            </div>
            <br><br>
            <div class="form-group form-inline">
                <select class="form-control spacer-right" id="CrJaNee" name="CrJaNee">
                    <option value="" selected>&lt;Selecteer een criterium&gt;</option>
                    <?php foreach ($crtsjanee as $crjanee) { ?>
                        <option value="<?= $crjanee['ID'] ?>"><?= $crjanee['Criterium'] ?></option>
                    <?php } ?>
                </select>
            ...
            <div class="form-group">
                <button type="submit" class="action-button" id="RID" name="RID" value="<?= $relatieid ?>" title="Deze criteria toevoegen.">Toevoegen</button>
